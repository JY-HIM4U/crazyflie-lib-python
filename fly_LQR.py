...
RL_DECISION_INTERVAL = 0.5   # RL decides every 0.5s; keep consistent with RL_smpc_config
MPC_PLANNING_INTERVAL = 0.025   # Must equal parameters.load_parameters() delta_t (LQR model step)
PHYSICS_TIME_STEP = 0.005      # Physics / sim only
...
def get_state_SI(uri: str) -> np.ndarray:
    """
    Returns the latest state for the given CF URI as a 13-dim numpy array and thrust:
    state: [x, y, z, qx, qy, qz, qw, vx, vy, vz, gyro_x, gyro_y, gyro_z]
    in SI units: m, m/s, rad, rad/s. Thrust is raw stabilizer value.
...
def send_trpy_command_pwm(cf, thrust_pwm: int, rollrate: float, pitchrate: float, yawrate: float, duration: float):
    ...
    time.sleep(duration)
def state_13_to_state_9(state_13: np.ndarray) -> np.ndarray:
    """
    Convert get_state_SI 13D output to 9D SMPC state [x, y, z, vx, vy, vz, roll, pitch, yaw].
    state_13: [x, y, z, qx, qy, qz, qw, vx, vy, vz, gyro_x, gyro_y, gyro_z]
    """
    qx, qy, qz, qw = float(state_13[3]), float(state_13[4]), float(state_13[5]), float(state_13[6])
    ...
    return np.array([
        state_13[0], state_13[1], state_13[2],
        state_13[7], state_13[8], state_13[9],
        roll, pitch, yaw
    ], dtype=np.float64)


def thrust_pwm_to_percentage(thrust_pwm: float) -> float:
    """Convert Crazyflie thrust PWM (10001..60000) to 0--100 for send_setpoint_manual."""
    pwm_min, pwm_range = 10001.0, (60000.0 - 10001.0)
    return float(np.clip((thrust_pwm - pwm_min) / pwm_range * 100.0, 0.0, 100.0))
...
def shrinking_horizon_SMPC_phoenix(cf, uri: str,
    ...
    for j in range(mpc_params['J']):
        ...
        smpc_next_state, thrust = get_state_SI(uri)  # 13D state; convert to 9D SMPC if needed
...
def test(cf, uri: str, initial_system_state=np.zeros((9,), dtype=np.float32)):
    ...
    # Load LQR/MPC parameters for manual setpoint control
    _com_params, _dp_params, mpc_params, _sim_params = load_parameters()
    A_mpc = np.asarray(mpc_params['A'], dtype=np.float64)
    B_mpc = np.asarray(mpc_params['B'], dtype=np.float64)
    Q_mpc = np.asarray(mpc_params['MPC_Q'], dtype=np.float64)
    R_mpc = np.asarray(mpc_params['MPC_R'], dtype=np.float64)
    J_mpc = int(mpc_params['J'])
    K_seq, F_seq, s_seq = None, None, None
    ...
    while time.time() - start_time < FLIGHT_DURATION:
        ...
        if current_time - last_action_time >= RL_DECISION_INTERVAL-0.1:
            ...
            for i in range(mpc_horizon):
                ...
                trajectory[0:3, i+1] = trajectory[0:3, i] + velocity_cmd * MPC_PLANNING_INTERVAL * ACTION_SCALE
                trajectory[3:6, i+1] = velocity_cmd * ACTION_SCALE
                # Keep other states constant (simplified)
            # Build reference (9, J+1) for LQT from trajectory
            reference_mpc = np.zeros((9, J_mpc + 1), dtype=np.float64)
            for k in range(J_mpc + 1):
                idx = min(k, trajectory.shape[1] - 1)
                reference_mpc[0:3, k] = trajectory[0:3, idx]
                reference_mpc[3:6, k] = trajectory[3:6, idx]
                reference_mpc[6:9, k] = 0.0
            lqt = compute_lqt_gains(A_mpc, B_mpc, Q_mpc, R_mpc, reference_mpc, Q_terminal=Q_mpc)
            K_seq, F_seq, s_seq = lqt['K_seq'], lqt['F_seq'], lqt['s_seq']
        ...
        if not switched_to_low_level:
            try:
                ...
                switched_to_low_level = True
            except Exception as e:
                print(f"Could not set low-level mode: {e}")
        state_13 = current_state
        state_9 = state_13_to_state_9(state_13)
        state_9[2] = DEFAULT_HEIGHT
        u_hat = lqt_control_step(K_seq[0], F_seq[0], s_seq[1], state_9)
        print("trajectory[0:2, -1]",trajectory[0:2, -1])
        print("state_9[0:2]",state_9[0:2])
        print("u_hat",u_hat)
        x_pred_next = A_mpc @ state_9 + B_mpc @ u_hat
        # Accumulate residual (real minus predicted XY); belief updated at RL decision step
        

        # Convert u_hat to thrust (PWM) and roll/pitch/yaw rates (rad/s then to deg/s for commander)
        T_max = 0.027 * 9.81 * 2.25
        hover_thrust = 0.027 * 9.81
        thrust_force = u_hat[0] * T_max
        thrust_cmd = thrust_to_cmd(hover_thrust + thrust_force)
        thrust_cmd = int(np.clip(thrust_cmd, 0, 65535))
        rate_scale = 1.0 / (np.pi / 3.0)  # rad/s from controller
        roll_rad_s = u_hat[1] * rate_scale
        pitch_rad_s = u_hat[2] * rate_scale
        yaw_rad_s = u_hat[3] * rate_scale
        roll_rad_s, pitch_rad_s, yaw_rad_s = np.clip([roll_rad_s, pitch_rad_s, yaw_rad_s], -2.0, 2.0)
        roll_deg_s = float(roll_rad_s * 180.0 / np.pi)
        pitch_deg_s = float(pitch_rad_s * 180.0 / np.pi)
        yaw_deg_s = float(yaw_rad_s * 180.0 / np.pi)
        thrust_pct = thrust_pwm_to_percentage(thrust_cmd)
        cf.commander.send_setpoint_manual(roll_deg_s, pitch_deg_s, yaw_deg_s, thrust_pct, rate=True)
        print(f"Time send command: {time.time() - send_command_time:.3f}s")
        send_command_time = time.time()
        ...
        record_sample(uri, current_state, thrust, {
                ...
                "acc_y": float(acc[1]),
                "acc_z": float(acc[2]),
                "residual_x": float(residual_xy_accumulated[0]),
                "residual_y": float(residual_xy_accumulated[1]),
        ...
        # Sleep to maintain loop rate (use MPC_PLANNING_INTERVAL so control runs on time)
        elapsed = time.time() - loop_start_time
        
        sleep_time = max(0, min(LOG_INTERVAL, MPC_PLANNING_INTERVAL) - elapsed)
    ...
    if switched_to_low_level:
        try:
            ...
            pass
    current_state, _ = get_state_SI(uri)  # 13D state
...
if __name__ == '__main__':
    try:
        ...
        print("\nInterrupted by user (Ctrl+C); saving logs and shutting down...")
    finally:
        # Always save logs (normal exit or Ctrl+C)
